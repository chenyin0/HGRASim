...
//		return arbitrator->AddTrans(input.value_addr, TAG, input.valid, input.tag);
	...
	void Lsu::update()   //从后往前，并加入timing信息
	{
		...
		//			{
		//				(arbitrator->ArbitratorLines[post_table[miss_index]->offset[i].pointer]->lse_)->LEcallback(post_table[miss_index]->ADDR_ + i, ClockCycle, post_table[miss_index]->offset[i].tag);
		...
		//	{
		//		bool find_offset = 0;
		//		for (short i = 0; i < system_parameter.post_offset_depth; i++)
		//		{
		...
		//					{
		//						(arbitrator->ArbitratorLines[post_offset[miss_index][i].offset[j].pointer]->lse_)->LEcallback(post_table[miss_index]->ADDR_ + j, ClockCycle, post_offset[miss_index][i].offset[j].tag);
	...
	for (int times = 0; times < CACHE_BANK; times++)
	{
		if (!pre_fifo[bank].empty())
		{
			...
			else                                                             //读请求
			{
				...
				{
					if (NotSameBlock((pre_fifo[bank].front()->ADDR_)))///////////////////////////////非同一block?????/////////
					{
